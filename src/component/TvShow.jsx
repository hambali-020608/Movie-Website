...
const movieList = async () => {
  try {
    const response = await axios.get(`${filmUrl}/tv/popular?api_key=${Api}`)
      ...
      return (
        <div className='col-lg-3 col-md-4 col-sm-6 col-xs-12  ' key={movie.id}  >
          <div className="card bg-dark bg-gradient text-white" style={{ marginBottom: '20px' }} data-aos="flip-right" data-aos-duration='1000'>
            <img src={`${film}/${movie.poster_path}`} className="card-img-top" alt={movie.name} />
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                <div className="modal-header">
                  <h1 className="modal-title fs-5" id={`${modalId}Label`}>{movie.name}</h1>
                    ...
                    <div className="row">
                      ...
                      <div className="col-md">
                        <ul className="list-group">
                          <li className="list-group-item"><h4>{movie.name}</h4>{movie.release_date}</li>
...
<div className="row">
  ...
  <input type="text" className="form-control input-keyword" id="halo"placeholder="Cari Tv Show..." aria-label="Recipient's username" aria-describedby="button-addon2"/>
